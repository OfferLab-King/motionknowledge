...
export async function POST(_request: Request, {params}: {params: Promise<{projectId: string; sceneId: string}>}) {
  ...
  try {
    const {jobId} = await enqueueSceneNarration(db, {workspaceId, projectId, sceneKey: sceneId});
    track({event: 'narration_regenerated', userId: user.id, workspaceId, projectId, properties: {sceneId}});
